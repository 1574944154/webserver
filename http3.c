...
PROCESS rio_read(rio_t *rio)
{
    ...
    {   
        ...
        if(read_size<=0)
        {
            ...
            {
                printf("read no data, count %d\n", count);
                noblocksleep(count*100);
                if(count==100) return PROCESS_ERR;
                continue;
            }
            return PROCESS_ERR;
        }else{
...
int parse_requestline(rio_t *rio, request_t *request)
{
    ...
    request->uri = (char *)malloc(p-rio->rio_buf_ptr);
    *(p-1) = 0;
...
void init(int port, char *host)
{
    ...
    while(1)
    {
        int sockcli=accept(socksev, (struct sockaddr*)&addrcli, &addrlen);
        printf("socket %d connected, the %d request\n", sockcli, ++count);
        setnonblocking(sockcli);
        process_request(sockcli);
